...
class Success extends StatelessWidget {
  ...
  Future SetPayment(String userid) async {
    CollectionReference users = FirebaseFirestore.instance
        .collection('User')
        .doc(userid)
        .collection('Reservation')
        .doc(reservationData.BookingID)
        .collection('Payment');

    DocumentReference newDocRef =
        users.doc(); // generates a new DocumentReference with a unique ID
    String newDocId = newDocRef.id; // gets the ID of the new document
    await newDocRef.set(paymentData.CreatePaymenttoJson());
